...
export const TestPage = () => {
    ...
    const [remaining, setRemaining] = useState<number>(() => parseDurationToSeconds(test?.duration));
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white p-6">
            ...
            <div className = "flex items-center mb-2">
                <div>
                    <h1 className="text-3xl font-bold mb-1">{test?.title ? `${test.title}` : `Test #${id}`}</h1>
                    {test && (
                        <p className="text-slate-400">{test.subject} • {test.duration} • {test.questions} questions</p>
                    )}
                </div>
